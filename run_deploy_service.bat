...
IF %1=="" (
	echo "You need to specify the environment first parameter"
	exit /b 1
)
...
IF %SERVICE_FILE_NAME%=="" (
	echo "You need to specify the service filename as the second parameter"
	exit /b 1
)

IF %SERVICE_NAME%=="" (
	echo "You need to specify the service name as the third parameter"
	exit /b 1
)
